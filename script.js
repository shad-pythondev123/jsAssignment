const data = [
  ...
  function concatenateArray() {
    const newData = [{name:"alex", age:22, profession: "designer"}, {name:"mary", age:29, profession: "developer"},];
    const concatenatedData = data.concat(nweData);
  ...
  function uniqueProfessions() {
    const professions = [newSet(data.map((person) => person.profession))];
  ...
  function getTotalProfessions() {
    const professionCount = data.reduce((count, person) => {count[person.profession] = (count[person.profession] || 0) + 1;
    return count;
  },{});
